...
byte NUM_MAX0 = 4;
byte NUM_MAX1 = 4;
byte displayTime = 0;    // 0-ЧЧ:ММ, 1-ЧЧ:ММ:СС.
...
int displayDot = 1;                      // 0 - Мигающие, 1 - Прыгающие
long localEpoc = 0;
...
float hT0 = 0.0; // влажность на улице с ThingSpeak со знаком и плавающей запятой
...
void showAnimClock() {
  if(displayTime == 0) showAnimClock0();
  else if(displayTime == 1) showAnimClock1();    
}
...
void sensors() {
  ...
  hT0 = humThink;
  if (humThinkOnOff) hT0 += corrhumThink;
  hT1 = int(hT0);
  hT2 = int(hT0 * 10 * (hT0 > 0 ? 1 : -1)) % 10;
  ...
  if (printCom) {
    ...
    Serial.println("          humThink = " + String(hT0)+ " %");
  }
}
//--------------------------------------------------------------------------
